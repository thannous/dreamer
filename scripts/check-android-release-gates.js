...
function checkAndroidReleaseGates({
  ...
  addCheck(
    ...
    'Upload an AAB, connect a physical tester device, install via Play Internal Testing, then run npm run android:play-qa-device:wait for one device, npm run android:play-qa-device:wait -- --device <adb-id> when multiple devices are ready, or npm run android:play-qa-device -- --device <adb-id>. Before UI-driven purchase or restore flows, run npm run android:play-qa-device:wait -- --device <adb-id> --require-ui-ready. After the preflight passes, test offering load, purchase, and restore.'
  );
