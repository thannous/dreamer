...
export default function LucidRealityCheckScreen() {
  ...
  const [method, setMethod] = useState<LucidRealityCheckMethod>('nose_breathing');
  const [context, setContext] = useState<LucidRealityCheckContext>('scheduled');
  const [outcome, setOutcome] = useState<LucidRealityCheckOutcome>('awake');
  const [mindful, setMindful] = useState(false);
  const [saving, setSaving] = useState(false);
  const close = () => closeLucidRoute(router, LUCID_HOME_HREF);
  const save = async () => { setSaving(true); try { await addRealityCheck({ method, context, outcome, mindful }); Alert.alert(copy.saved, content.realityChecks.completionPrompt, [{ text: content.chrome.common.done, onPress: close }]); } finally { setSaving(false); } };
  return (
    <LucidScreen eyebrow={copy.eyebrow} title={copy.title} subtitle={copy.subtitle} trailing={<LucidIconAction label={content.chrome.common.cancel} icon="close" onPress={close} />} testID="lucid-reality-check">
      ...
      <LucidButton label={copy.save} icon="checkmark" disabled={!mindful} loading={saving} onPress={() => void save()} testID="lucid-reality-save" />
    </LucidScreen>
  );
}
