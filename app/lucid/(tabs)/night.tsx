...
export default function LucidNightScreen() {
  ...
  const start = async () => {
    if (!safe) { Alert.alert(copy.safety, copy.blocked); return; }
  ...
  return (
    <LucidScreen testID="lucid-night" eyebrow={copy.eyebrow} title={copy.title} subtitle={copy.subtitle}>
      <LucidCard accent={audio.plan ? 'cyan' : 'violet'}>
        ...
        {audio.plan ? <LucidButton label={copy.stop} variant="danger" icon="stop" onPress={() => void audio.stopNight()} /> : <LucidButton label={copy.start} icon="moon" disabled={!safe || audio.isScheduling} onPress={() => void start()} testID="lucid-night-start" />}
