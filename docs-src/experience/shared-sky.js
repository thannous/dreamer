/** One viewport-sized composited backdrop, synchronized with the opening film. */
function createSkyMotion(doc, shared, media, main, dreams) {
  ...
  const blocked = () => !onScreen || win.__EXP_TIER__ === 'static' || reduced.matches || doc.hidden || doc.documentElement.classList.contains('oh-lightbox-open') || doc.querySelector('.oh-intro-overlay') || doc.documentElement.classList.contains('exp-intro-pending');
  const schedule = () => { if (!raf && !blocked()) raf = win.requestAnimationFrame(tick); };
  function tick(now) {
    raf = 0;
  ...
  const onScroll = () => {
    const y = win.scrollY;
    target = clamp((y - top) / range);
    // Fade only the viewport surface when leaving the whole journey. Never mask
    // a seven-screen ancestor: WebKit otherwise has to tile a huge video layer.
    const opacity = clamp((end - y) / height);
    shared.style.opacity = String(opacity);
    const visible = opacity > 0;
    if (visible !== onScreen) {
      onScreen = visible;
      shared.style.visibility = visible ? '' : 'hidden';
      doc.dispatchEvent(new win.Event('dream-sky-visibility-change'));
    }
    schedule();
  };
  const refresh = () => {
    height = shared.clientHeight || win.innerHeight;
    top = main.getBoundingClientRect().top + win.scrollY;
    const rect = dreams.getBoundingClientRect();
    end = rect.bottom + win.scrollY;
    range = Math.max(1, end - top - height);
    onScroll();
  };
  const reset = () => {
...
export function initSharedSky() {
  ...
  shared.className = 'oh-shared-sky'; shared.setAttribute('aria-hidden', 'true');
  const poster = doc.createElement('div'); poster.className = 'oh-sky-poster';
  const media = doc.createElement('div'); media.className = 'oh-sky-media';
  poster.append(media); shared.append(poster); main.prepend(shared);
  main.classList.add('oh-has-shared-sky');
  const motion = createSkyMotion(doc, shared, media, main, dreams);
  const placeFilm = () => {
  ...
  const observer = new MutationObserver(records => {
    for (const record of records) {
      for (const node of record.addedNodes) {
        if (node.classList?.contains('oh-hero-film')) placeFilm();
  ...
  const sizes = new ResizeObserver(motion.refresh);
  sizes.observe(header); sizes.observe(dreams);
  placeFilm();
}
