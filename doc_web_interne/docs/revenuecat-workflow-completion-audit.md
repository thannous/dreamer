...
| Valider switch compte apres achat | QA Lab + evidence gate `account_switch`; guard local `maestro/subscription-teststore-signout-guard.yml` | Guard local passe le 2026-05-14 apres restore: l'app revient a l'etat de connexion, `Continue with Google` est visible, `Noctalia Plus is active` et `plus / active` ne sont pas visibles. La preuve complete exige encore un deuxieme compte reel. | Partiel local, second compte manquant |
...
| Verifier les preuves locales manuelles | `doc_web_interne/docs/revenuecat-qa-evidence.local.json` | Fichier local gitignore cree le 2026-05-14 avec `test_store_monthly`, `test_store_annual` et `restore_after_reinstall` passes pour l'appUserId RevenueCat `1239729f-7468-48c9-b26a-7aa8b4a82591`; `subscription:qa:report` affiche 3 scenarios manuels verifies et 4 gates restantes | Partiel local |
...
| Enregistrer les preuves | `revenuecat-qa-evidence.example.json`, `update-subscription-qa-evidence.js` | Tests prouvent evidence -> report -> gate count; les gates Play sans `easBuildId` UUID et les preuves sans `appUserId` UUID restent bloquees; les textes de preuve restes au template, les dates invalides, les identites en espaces et les JSON invalides ne comptent pas. Le 2026-05-14, l'appUserId RevenueCat a ete extrait des preferences app debug et les preuves locales Test Store monthly, annual et restore apres reinstall ont ete inscrites dans le fichier gitignore |
...
- `subscription:qa:verify-local`: passe, avec 5 suites et 35 tests du harnais QA scripts; son sous-test release gate isole volontairement les preuves locales et attend donc encore le blocage "7 gates" sans fichier local
