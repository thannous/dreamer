...
- preuve locale: `doc_web_interne/docs/revenuecat-qa-evidence.local.json` absent; malgre les achats Test Store locaux, le restore local et le guard logout/no-leak passes, `subscription:qa:release-gate` doit rester rouge tant que les preuves structurees avec `appUserId`, la preuve account-switch a deux comptes et les gates Play ne sont pas renseignees
