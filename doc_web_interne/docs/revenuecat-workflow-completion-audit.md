...
| Revalider Play apres build 24 | Google Play sur `Pixel_8_Play_API_36` + `adb exec-out uiautomator dump /dev/tty` | La fiche Play affiche toujours `Your device isn't compatible with this version` et `You're a beta tester for this app`; Noctalia n'est pas installee | Bloque Play/device catalog |
