...
| Verifier l'etat store live des produits Play | RevenueCat MCP `get_product_store_state` via `codex exec` frais | Relu le 2026-05-14: `prod98337b31be` annual renvoie `base_plan_id=annual`, `billing_period=P1Y`, `status=ok / ACTIVE`; `prodfce10ef2a8` monthly renvoie aussi `base_plan_id=annual`, `billing_period=P1Y`, `status=ok / ACTIVE`, ce qui indique une incoherence Play/RevenueCat a corriger avant de valider l'achat mensuel reel | Bloque configuration Play monthly |
...
- RevenueCat MCP store state: lecture fraiche le 2026-05-14 avec `codex exec` et `get_product_store_state` sur `prodfce10ef2a8` et `prod98337b31be`; les deux produits Play renvoient `store=play_store`, `status=ok / ACTIVE`, `base_plan_id=annual`, `billing_period=P1Y`. Le produit mensuel est donc toujours incoherent cote store live; le code local evite de l'afficher comme mensuel si `subscriptionPeriod=P1Y`, mais la configuration Play/RevenueCat doit etre corrigee avant de valider `play_monthly`.
