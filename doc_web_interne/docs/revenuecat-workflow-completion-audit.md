...
| Filtrage hardware Play | `withOptionalAndroidHardwareFeatures` | Correctif local ajoute pour marquer camera/micro optionnels dans le manifest prebuild; verifier apres un nouveau build Store et une nouvelle soumission Internal Testing | Nouveau build requis |
...
- Play Internal Testing: build Android Store `cbbf745a-0e76-4488-a365-ba180a903e90` cree depuis `723e6deb` avec la cle `goog_`; auto-submit Internal Testing `6d4630b0-33e1-48d7-a8f1-687206b2fe8f` confirme `Success` cote Expo; `thannous@gmail.com` est inscrit comme testeur et connecte dans l'AVD Google Play local; la fiche Play Noctalia Beta est visible, mais l'installation est bloquee par `Your device isn't compatible with this version`; bundletool genere pourtant des splits APK pour cet emulateur depuis l'AAB, donc le blocage restant semble cote Play/device catalog plutot que cote manifest installable; un correctif local marque maintenant camera/micro optionnels, mais il doit passer par un nouveau build Store avant de pouvoir changer la compatibilite Play
