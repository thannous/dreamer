...
2. Manual retry previously trusted the same cached connectivity flag that could have suppressed the original replay. Explicit retry and foreground recovery now query native connectivity, with a five-second probe deadline, then bind that attempt to the fresh result. A save and its queued follow-up edits with a cached offline state also check again. A dedicated follow-up-edit regression fails without the latter wakeup and passes with it. Actual offline state preserves the queue and manual retry displays a translated failure message.
...
- Focused hook, engine and route tests: 174 passed initially; the final follow-up-edit delta passed all 90 journal-hook tests.
...
- `npm run test:prepush` passed on clean code commit `bcb8479e6b40851743aadf2898997c2c4a6aa1a7`: 111 suites / 1,105 tests, app/test types; fresh base `214c6e845e73599f0ee61cfa535303e2cfa73717`.
