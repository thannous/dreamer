...
After the real run above, updateCopy was changed to reread and confirm the exact intended durable snapshot when an atomic write acknowledgement fails. The RPC reader and SQL contract are unchanged. A parallel correction was reconciled by sharing the persistence helper between page checkpoints and copy updates, with idempotent repeated deletion/conflict decisions. Final guards reject cancellation between helper continuations. The engine file hash above identifies the real-run source, not this later full file. Independent review and 58 focused tests cover committed decisions, unapplied writes, retry, account changes and the reproduced microtask cancellation race. No additional database run is claimed for that local persistence correction.
