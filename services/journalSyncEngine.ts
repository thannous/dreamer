/** Durable Journal replay engine. No React, native storage or network imports. */
...
import { type DreamListUpdater, generateMutationId, getMutationDreamTarget, getMutationRemoteId, upsertDream } from '../lib/dreamUtils';
import type { recordSyncReplayMetrics as recordMetrics, reportSyncQueueMetrics as reportMetrics } from '../lib/syncObservability';

export type JournalSyncOptions = {
...
export type JournalSyncDependencies = {
  savePendingDreamMutations: (mutations: DreamMutation[], scope?: string | null) => Promise<void>;
  createDreamInSupabase: (dream: DreamAnalysis, userId: string) => Promise<DreamAnalysis>;
  updateDreamInSupabase: (dream: DreamAnalysis) => Promise<DreamAnalysis>;
  deleteDreamFromSupabase: (remoteId: number) => Promise<void>;
  syncDreamMutationsInSupabase?: (mutations: DreamMutation[], userId: string) => Promise<SyncMutationResult[]>;
  reportSyncQueueMetrics: typeof reportMetrics;
  recordSyncReplayMetrics: typeof recordMetrics;
  logger: { warn: (message: string, error?: unknown) => void };
};

export function matchesInitialJournalScope({ initialMutations = [], initialMutationsLoaded = true,
  userScope, initialMutationsScope = userScope }: JournalSyncOptions): boolean {
  return initialMutationsLoaded && initialMutationsScope === userScope &&
    initialMutations.every((mutation) => !userScope || !mutation.userScope || mutation.userScope === userScope);
}

const cell = <T>(current: T): { current: T } => ({ current });

export function createJournalSyncEngine(dependencies: JournalSyncDependencies, initialOptions: JournalSyncOptions) {
  const { savePendingDreamMutations, createDreamInSupabase, updateDreamInSupabase,
    deleteDreamFromSupabase, syncDreamMutationsInSupabase, reportSyncQueueMetrics,
    recordSyncReplayMetrics, logger } = dependencies;
  const { userScope, initialMutations = [] } = initialOptions;
  const initialSnapshotMatchesScope = matchesInitialJournalScope(initialOptions);
  const pendingMutationsRef = cell<DreamMutation[]>(
  ...
  const activeUserScopeRef = cell(userScope);
  const mutationWriteTailsRef = cell<Map<string, Promise<void>>>(new Map());

  const activate = (userScope: JournalSyncOptions['userScope'], initialSnapshotMatchesScope: boolean) => {
  ...
  // Bindings create closures only. Scope changes become active after React commits.
  const bindPersistence = ({ canUseRemoteSync, userScope }:
    Pick<JournalSyncOptions, 'canUseRemoteSync' | 'userScope'>) => {
    const persistPendingMutations = async (mutations: DreamMutation[]) => {
    ...
    const setPendingMutations = (mutations: DreamMutation[]) => {
    ...
    const appendPendingMutation = async (mutation: DreamMutation) => {
    ...
    const queueOfflineOperation = async (mutation: DreamMutation, updater: DreamListUpdater, persistRemoteDreams: JournalSyncOptions['persistRemoteDreams']) => {
      await appendPendingMutation(normalizeMutation(mutation, userScope));
      await persistRemoteDreams(updater);
    };

    const clearQueuedMutationsForDream = async (dreamId: DreamTarget): Promise<boolean> => {
    ...
    const retryDreamMutations = async (dreamId: DreamTarget): Promise<boolean> => {
      if (typeof dreamId === 'number') {
        const keys = new Set(pendingMutationsRef.current.filter((entry) =>
          matchesDreamTarget(getMutationDreamTarget(entry), dreamId)
        ).map((entry) => getDreamIdentityKey(getMutationDreamTarget(entry))));
    ...
    const hydrate = (initialMutations: DreamMutation[], initialSnapshotMatchesScope: boolean) => {
    ...
    return {
      pendingMutationsRef, persistPendingMutations, queueOfflineOperation,
      clearQueuedMutationsForDream, retryDreamMutations, generateMutationId,
      setPendingMutations, hydrate
    };
  };

  const bind = ({ canUseRemoteSync, hasNetwork, userScope, persistRemoteDreams }:
    Pick<JournalSyncOptions, 'canUseRemoteSync' | 'hasNetwork' | 'userScope' | 'persistRemoteDreams'>,
    ...
    const subscribeSnapshot = (remoteSnapshot: JournalSyncOptions['remoteSnapshot'], initialSnapshotMatchesScope: boolean) => {
    ...
    return {
      ...persistence,
      queueOfflineOperation: (mutation: DreamMutation, updater: DreamListUpdater) =>
        persistence.queueOfflineOperation(mutation, updater, persistRemoteDreams),
      syncPendingMutations, subscribeSnapshot
    };
  };
  return { bindPersistence, bind, activate, setMounted: (mounted: boolean) => { mountedRef.current = mounted; } };
}
