...
describe('LucidCard press feedback', () => {
  ...
  it('forwards labels and test IDs on a passive card', () => {
    const { getByTestId } = render(
      <LucidCard accessibilityLabel="Saved weekly review" testID="weekly-review">
        Card
      </LucidCard>
    );

    const card = getByTestId('weekly-review');
    expect(card.getAttribute('data-accessibility-label')).toBe('Saved weekly review');
...
describe('LucidButton disabledReason', () => {
  ...
  it('keeps an explicit accessibilityHint while showing the disabled reason inline', () => {
    const { getByTestId, getByText } = render(
      <LucidButton
