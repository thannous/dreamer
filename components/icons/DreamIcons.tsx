...
function useFallbackIconColor(color?: string) {
  const { colors, mode } = useTheme();
  const noctalia = getNoctaliaDesignTokens(colors, mode);
  return color ?? noctalia.text.primary;
}

// Flight/Airplane icon - for lucid dreams, flying
export function PlaneIcon({ size = 24, color }: IconProps) {
  const iconColor = useFallbackIconColor(color);
...
export function BrainIcon({ size = 24, color }: IconProps) {
  const iconColor = useFallbackIconColor(color);
...
export function BoltIcon({ size = 24, color }: IconProps) {
  const iconColor = useFallbackIconColor(color);
...
export function DreamIcon({ size = 24, color }: IconProps) {
  const iconColor = useFallbackIconColor(color);
...
// Eye off icon - for hidden content
export function EyeOffIcon({ size = 24, color, slashColor }: EyeOffIconProps) {
  const iconColor = useFallbackIconColor(color);
