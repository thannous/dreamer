...
- A competitor-citation gap pass originally surfaced Dealroom and Simone.
  Dealroom's followed competitor pattern and Dutch operator passed research,
  but the user explicitly abandoned it at 03:46 CEST; it is now P2 and closed
  with no account, demo or contribution. Simone remains P1: its April 2026
  Rosebud review links the product domain twice without `nofollow`; Agentik SAS
  and French law provide accountable non-Russian control, and one factual
  editorial note is prepared but unsent.
...
These choices are deliberately separate so approval for one route cannot be
misread as approval for another.

### D1 — Elsewhere factual reply

> J'autorise l'envoi de la réponse factuelle préparée à Elsewhere : Noctalia est
> disponible sur Android, sans date iOS publique et sans export complet
> PDF/CSV/JSON, sans demande de lien ni de placement.

### D2 — Marika Pech correction and mention reclamation
...
The user explicitly abandoned Dealroom on 2026-08-10. No account, demo,
submission or payment is due. `D8A` is retired unless a new explicit user
instruction reopens the route.
...
2. Record only the authoritative sent or form-accepted state; do not infer
   delivery or editorial acceptance.
3. If a publisher replies, stop automation and review the human response before
   any further transmission.
4. Record a backlink only after a public referring page exposes a clickable
   Noctalia destination and its HTTP status, canonical, indexability and link
   attributes have been inspected.
