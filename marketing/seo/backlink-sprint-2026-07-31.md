...
An authenticated Gmail search found one material human editorial response. Dan Kennedy of Elsewhere replied that his next dream-app comparison is now likely in 2027, that he will reconsider Noctalia, and asked whether an iOS version is planned and whether the journal supports export. Current product evidence supports only an honest limited answer: Noctalia is publicly available on Android, is not currently available on iOS, has no public iOS release date, can share an individual dream card as a JPEG and can back up journal data through an account, but has no full journal PDF, CSV, JSON or archive export. The English press kit now publishes those current facts at `/en/press#current-availability`, creating a stable source for editorial verification without inventing an iOS roadmap commitment. The exact reply is prepared in `marketing/seo/editorial-response-elsewhere-2026-08-09.md`; it remains unsent pending the founder's answer on whether iOS is planned and explicit send authorization. No future follow-up date is retained while a human question is open.
...
### Editorial discovery wave 32 — 2026-08-09

Two current AI-journaling privacy comparisons were rejected before contact.
Dandelion Reflect has a named India-based author, an explicit factual-correction
invitation and a directly relevant July 2026 audit, but every inspected external
source citation is `nofollow`. Daylogue is operated by an accountable California
LLC and its comparison is directly relevant, but all named competing products
remain plain text and the page's only external anchors lead to Daylogue's own
social profiles. Neither page demonstrates a followed competitor-domain citation
path, so no email, form, account, product data or payment was used. Full evidence
is recorded in `marketing/seo/editorial-discovery-wave-32-2026-08-09.md`; the
central prospect register now contains 194 routes.

### Unlinked brand mention reclamation — 2026-08-09

Marika Pech's current French article on precognitive dreams is a new P0 source-
correction route. The exact self-canonical, index-follow page attributes the
unsupported `40–60 %` experience figure to Noctalia in plain text without a
link. Its legal notice identifies Marika Pech as a French auto-entrepreneur in
Castelnau-le-Lez and OVH France as host; no Russian operator signal was found.

Noctalia's cited French source has been corrected to distinguish belief from
self-reported experience, cite the peer-reviewed Valášek and Watt (2015) paper
and remove an unverifiable direct quotation attributed to Caroline Watt. The
reclamation dossier proposes a matching factual correction and asks only that
the existing `Source : Noctalia` attribution link to the corrected article. The
publisher page demonstrates no external editorial source-link pattern, so
acceptance and link treatment remain unknown. No form, email, comment, account
or product data was used; transmission requires explicit authorization after
the corrected Noctalia source is publicly verified. The central register now
contains 195 routes: 3 P0, 63 P1, 128 P2 and 1 P3. Full evidence and unsent copy
are recorded in
`marketing/seo/brand-mention-reclamation-marikapech-2026-08-09.md`.

### Editorial discovery wave 33 — 2026-08-09

Two French pages ranking for precognitive-dream queries were rejected before
contact. Doctissimo's current legal notice identifies MARMITON SAS and the
Reworld Media group in France, and its editorial charter invites factual
corrections; however, every inspected external expert-domain anchor on the exact
article is `nofollow`, while its academic references are plain text.
Sommeil.org is operated by 99 DIGITAL SAS under a current Paris registration
and Levallois-Perret address, but the exact topical article demonstrates no
external editorial source citations. Both pass the accountable non-Russian
operator gate and fail the followed-citation gate. No email, form, account,
comment, product data or payment was used. The central register now contains
